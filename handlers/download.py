...
def _err(e):
    m = {"ENV_ERROR":"Error temporal.","YOUTUBE_BLOCK":"Bloqueado. Usa /cookies.","VIDEO_PRIVADO":"Privado.","VIDEO_NO_DISPONIBLE":"No disponible.","FORMATO_NO_DISPONIBLE":"Formato no disponible.","ARCHIVO_MUY_GRANDE":"Excede 300MB.","TIMEOUT":"Tardo mucho. Intenta otra calidad."}
    for k,v in m.items():
        if k in e: return "❌ " + v
    return "❌ " + e[:100]
...
async def handle_tiktok(up, ctx, url):
    ...
    if durl:
        try: await s.edit_text("📤 Enviando..."); await up.message.reply_video(durl,caption="✅ TikTok",supports_streaming=True); await s.delete(); cleanup(path) if path else None; return ConversationHandler.END
        except: pass
    ...
    try:
        ...
        with open(path,"rb") as f: await up.message.reply_video(f,caption="✅ TikTok",supports_streaming=True)
        await s.delete()
